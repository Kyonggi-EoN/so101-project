...
1. 윈도우 탐색기로 `so101_project` 폴더에 들어간다
2. 주소창에 `powershell` 이라고 치고 엔터

그 폴더에서 시작하는 PowerShell 창이 열린다. **항상 `so101_project` 폴더에서 명령을 실행해야 한다.** 다른 데서 실행하면 파일을 못 찾는다.
...
1. `so101_project\.venv\` 폴더가 생긴다 — 여기가 이 프로젝트 전용 패키지 보관함이다
